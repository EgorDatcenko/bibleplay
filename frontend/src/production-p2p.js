// Production P2P решение для браузера
class ProductionP2P {
  constructor() {
  ...
  async createRoom(roomId, playerName) {
    try {
      ...
      console.log('[Production-P2P] Комната создана:', roomId);
      return { success: true, roomId, gameState: this.gameState };
      
    } catch (error) {
      console.error('[Production-P2P] Ошибка создания комнаты:', error);
  ...
  async joinRoom(roomId, playerName) {
    try {
      ...
      console.log('[Production-P2P] Подключение к комнате:', roomId);
      
      return { success: true, roomId };
      
    } catch (error) {
      console.error('[Production-P2P] Ошибка подключения:', error);
      return { success: false, error: error.message };
    }
  }
  ...
  async createPeerConnection(peerId) {
    ...
    dataChannel.onopen = () => {
      console.log('[Production-P2P] Data channel открыт для пира:', peerId);
    ...
    dataChannel.onmessage = (event) => {
      try {
        ...
        console.error('[Production-P2P] Ошибка парсинга сообщения:', error);
      }
    };

    dataChannel.onerror = (error) => {
      console.error('[Production-P2P] Ошибка data channel:', error);
    };

    dataChannel.onclose = () => {
      console.log('[Production-P2P] Data channel закрыт для пира:', peerId);
    ...
    peerConnection.ondatachannel = (event) => {
      ...
      dataChannel.onopen = () => {
        console.log('[Production-P2P] Входящий data channel открыт');
        this.dataChannels.set(peerId, dataChannel);
      };
      ...
      dataChannel.onmessage = (event) => {
        try {
          ...
          console.error('[Production-P2P] Ошибка парсинга входящего сообщения:', error);
        }
      };
    };
    ...
    peerConnection.onconnectionstatechange = () => {
      console.log('[Production-P2P] Состояние соединения:', peerConnection.connectionState);
      
      if (peerConnection.connectionState === 'failed') {
        console.error('[Production-P2P] Соединение не удалось');
  ...
  handleConnectionFailure(peerId) {
    console.log('[Production-P2P] Обработка сбоя соединения для пира:', peerId);
  ...
  sendToPeer(peerId, message) {
    ...
    if (dataChannel && dataChannel.readyState === 'open') {
      try {
        ...
        console.error('[Production-P2P] Ошибка отправки сообщения:', error);
      }
    } else {
      console.warn('[Production-P2P] Data channel не готов для пира:', peerId);
    }
  }
    ...
    this.dataChannels.forEach((dataChannel, peerId) => {
      if (dataChannel.readyState === 'open') {
        try {
          ...
          console.error('[Production-P2P] Ошибка broadcast:', error);
        }
      }
    });
  }
  ...
  handleMessage(message) {
    console.log('[Production-P2P] Получено сообщение:', message);
    ...
    switch (message.type) {
      ...
      case 'answer':
        ...
        break;
      default:
        console.warn('[Production-P2P] Неизвестный тип сообщения:', message.type);
    }
  }
  ...
  emit(event, data) {
    if (!this.roomId) {
      console.error('[Production-P2P] Нет активной комнаты');
      return;
    }
