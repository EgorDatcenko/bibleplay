...
import P2PIntegration from './p2p-integration.js';
...
function App() {
  ...
  const [p2pIntegration, setP2pIntegration] = useState<P2PIntegration | null>(null);
  ...
  useEffect(() => {
    ...
    p2pIntegration.on('update', (data: any) => {
    ...
    p2pIntegration.on('lobbyUpdate', (data: any) => {
  ...
  const createRoomP2P = async () => {
    ...
    try {
      ...
      const result = await p2pIntegration.createRoom(playerName.trim());
  ...
  const playCardP2P = (cardId: number, insertIndex: number) => {
    ...
    p2pIntegration.playCard(cardId, insertIndex);
  };
